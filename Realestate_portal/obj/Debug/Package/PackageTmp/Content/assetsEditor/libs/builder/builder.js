				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							...
							item.css({
                                backgroundImage: "url(" + '/Content/assetsEditor/libs/builder/' + component.image + ")",
				...
				for (i in blocks)
				{
					...
					if (block)
					{
						...
						if (block.image) {
							...
							item.css({
                                backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? '/Content/assetsEditor/libs/builder/':'') + block.image + ")",
								backgroundRepeat: "no-repeat"
							})
						}
...
/* drag and drop */
	...
	saveAjax: function(fileName, startTemplateUrl, callback)
	{
        ...
        var template = {
            ...
            title: nombre,
            type: type,
            id_template: id_plantilla
        }

		$.ajax({
			type: "POST",
            url: '/CustomResources/guardarPlantilla',//set your server side save script url
            data: template,
			cache: false,
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url(/Content/assetsEditor/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
